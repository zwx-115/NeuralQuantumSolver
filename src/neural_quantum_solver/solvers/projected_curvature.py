...
class CurvatureSettings:
    ...
    def __post_init__(self) -> None:
        values = (self.radius, self.min_radius, self.max_radius, self.parameter_radius,
                  self.regularization, self.cg_rtol, self.accept_ratio, self.interior_fraction)
        ...
        if self.interior_fraction >= 1:
            raise ValueError("interior_fraction 必须小于 1，给非线性态距离留出余量")
...
class OverlapCurvature:
    ...
    def step(self) -> CurvatureStep:
        ...
        for trial in range(1, settings.max_trials+1):
            ...
            # 局部度量只是二阶近似；预留内边界，避免任意小的高阶项导致永远拒绝。
            local_radius = settings.interior_fraction*radius
            z = solve_trust_region(reduced_g, h, bounded_metric, local_radius)
            update = vmat @ z
            ...
            if predicted > 0:
                ...
                if math.isfinite(actual_loss) and math.isfinite(actual_distance):
                    ...
                    if rho >= settings.accept_ratio and actual_loss < before and actual_distance <= radius**2:
                        ...
                        if rho > .75 and float(z @ bounded_metric @ z) > .8*local_radius**2:
